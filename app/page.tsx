...
export default function LandingPage() {
  const { isAuthenticated, userType, logout } = useUserStore();

  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      {/* Navigation */}
      <nav className="border-b border-slate-200 dark:border-slate-800 sticky top-0 bg-white/95 dark:bg-slate-950/95 backdrop-blur z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-3 items-center">
            {isAuthenticated ? (
              <>
                <Link href={userType === 'CITIZEN' ? '/citizen/dashboard' : '/admin/dashboard'}>
                  <Button variant="ghost" className="font-medium">
                    Go to {userType === 'CITIZEN' ? 'Citizen' : 'Admin'} Portal
                  </Button>
                </Link>
